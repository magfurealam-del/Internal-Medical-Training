...
import { getPreviousAttempt } from "@/lib/training/courses";
...
export default async function QuizPage({ params }: { params: Promise<{ quizId: string }> }) {
  ...
  const [{ quiz, items }, previousAttempt] = await Promise.all([getQuiz(quizId), getPreviousAttempt(quizId)]);
  if (!quiz) notFound();

  // Resolve the module → course for breadcrumb back-link
    ...
    <main className="mx-auto max-w-4xl px-6 py-12 lg:px-10">
      <LearningBreadcrumbs
        items={[
          ...
          ...(courseId && moduleId ? [{ label: "Back to module", href: `/courses/${courseId}/modules/${moduleId}` }] : []),
          { label: "Assessment" },
        ]}
      />
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        {previousAttempt && (
          <span className={`rounded-full px-4 py-1.5 text-sm font-medium ${previousAttempt.passed ? "bg-[#e4f7ec] text-[#145c36]" : "bg-[#fff0ef] text-[#9d2c25]"}`}>
            Previous attempt: {previousAttempt.score_percentage}% — {previousAttempt.passed ? "Passed" : "Not passed"}
          </span>
        )}
      </div>
      ...
      {items.length === 0 ? (
        ...
        <QuizForm
          ...
          returnHref={courseId && moduleId ? `/courses/${courseId}/modules/${moduleId}` : "/dashboard"}
        />
      )}
    </main>
  );
