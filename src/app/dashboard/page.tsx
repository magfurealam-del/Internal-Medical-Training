          ...
          <div className="mt-5 grid gap-5 md:grid-cols-2">
            {coursesWithProgress.map(({ course, progress, next }) => (
              <Link key={course.id} href={`/courses/${course.id}`} className="group rounded-2xl bg-white p-6 ring-1 ring-[#d5e9ed] transition hover:ring-[#007c8b]">
                ...
                <div className="mt-5 h-2 rounded-full bg-[#d9f2f4]">
                  <div className="h-2 rounded-full bg-[#007c8b] transition-all" style={{ width: `${progress.percentage}%` }} />
                </div>
