...
export type Quiz = { id: string; title: string; description: string | null; pass_percentage: number; attempt_limit: number | null };

export async function getQuiz(quizId: string) {
  const supabase = await createSupabaseServerClient();
  const [{ data: quiz, error: quizError }, { data: items, error: itemsError }] = await Promise.all([
    supabase.from("quizzes").select("id, title, description, pass_percentage, attempt_limit").eq("id", quizId).maybeSingle(),
