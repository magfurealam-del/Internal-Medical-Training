...
begin
  ...
  if not exists (
    ...
    join training.quizzes q on q.course_id = e.course_id
    where q.id = p_quiz_id and e.user_id = v_user_id
      and e.status in ('active', 'completed')
  ) then
    raise exception 'Not enrolled in this quiz';
  end if;

  -- Enforce attempt_limit if set
  select attempt_limit into v_attempt_limit from training.quizzes where id = p_quiz_id;
  if v_attempt_limit is not null then
    select count(*) into v_attempt_count
    from training.attempts
    where quiz_id = p_quiz_id and user_id = v_user_id;
    if v_attempt_count >= v_attempt_limit then
      raise exception 'Attempt limit reached for this quiz';
    end if;
  end if;
